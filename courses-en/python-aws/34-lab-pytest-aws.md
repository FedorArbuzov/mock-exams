# 34. Lab: pytest suite with moto

## Scenario

We extend the stack tests: S3 (exists), DynamoDB CRUD, SQS send/receive, Lambda handler `hello`. Everything in-process with moto — **without** LocalStack for CI speed.

**Goal:** ≥4 test modules/files green in `pytest tests/ -v`.

---

## Step 1. conftest.py

Create `stack/tests/conftest.py`:
...
## Step 6. Marker integration (optional)
...
moto + pytest is the standard way to test boto3 wrappers. Patch settings, `@mock_aws`, test handlers as functions. Interview prep — next.
