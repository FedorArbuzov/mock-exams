# 17. DynamoDB: batch_write and transact_write

## Intro: "importing 500 items — 500 separate put_item calls"

A migration script calls `put_item` in a loop — slow, throttling. **batch_write_item** — up to 25 items per call. You need an atomic "debit the balance + create the order" — **transact_write_items**.

## What you'll learn

- `batch_write_item` limits and unprocessed keys.
...
| Mixed ops | Put + Delete in the same table |
...
**All-or-nothing** — up to 100 actions, 4 MB transaction payload:
...
| Failure | Effect |
|---------|--------|
| Any condition fails | the entire transaction is cancelled |
...
Related: [18-lab-idempotent-ddb](18-lab-idempotent-ddb.md).
...
LocalStack 3.x supports batch/transact — verify edge cases before prod.
...
| Float in a transact Item | ValidationException |
...
Next: [18-lab-idempotent-ddb](18-lab-idempotent-ddb.md).
