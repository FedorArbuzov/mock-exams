#!/usr/bin/env bash
# Reference only — run on the 16 GB host after ENVIRONMENT.md profile zbx exists.
set -euo pipefail

for i in 1 2 3; do
  name="node-0${i}"
  ip="192.168.61.$((9+i))"
  lxc launch ubuntu:22.04 "$name" --profile zbx
  lxc config device set "$name" eth0 ipv4.address="$ip"
  lxc config set "$name" limits.memory=512MiB
done
...
echo "SSH: ubuntu@192.168.61.10-12"
