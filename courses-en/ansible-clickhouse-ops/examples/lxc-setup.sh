#!/usr/bin/env bash
# Reference only — run on the 16 GB host after ENVIRONMENT.md profile ch exists.
set -euo pipefail

for i in 1 2 3; do
  name="ch-0${i}"
  ip="192.168.60.$((9+i))"
  lxc launch ubuntu:22.04 "$name" --profile ch
  lxc config device set "$name" eth0 ipv4.address="$ip"
  lxc config set "$name" limits.memory=2GiB
done
lxc restart ch-01 ch-02 ch-03

test -f ~/.ssh/id_ed25519 || ssh-keygen -t ed25519 -N "" -f ~/.ssh/id_ed25519

for name in ch-01 ch-02 ch-03; do
...
echo "SSH: ubuntu@192.168.60.10-12"
