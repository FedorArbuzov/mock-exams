...
kubectl create configmap app-config \
  --from-literal=GREETING=hi-from-lab \
...
Use this `pod-env.yaml` example (you can rename the Pod, but keep ConfigMap `app-config`):
...
**Check:** the logs show `GREETING=hi-from-lab` and `LOG_LEVEL=debug`.
...
```bash
kubectl create configmap nginx-page --from-literal=index.html='<h1>Lab page from ConfigMap</h1>'
```

Use this `pod-cm.yaml` example (feel free to add fields like resources/probes):
...
**Check:** you get back `<h1>Lab page from ConfigMap</h1>`.
...
Use this `secret-demo.yaml` example (you can keep or change container name/command style):
