...
**Start lab** applies this for you (or use `manifests/23-case1-broken.yaml`). Diagnose:
...
**Task:** find what's wrong in **Events**. Fix the image to `nginx:1.27` (`kubectl edit deploy/web`, or write your own `case1.yaml` and re-apply).
  ...
  containers:
    - name: web
      ...
      readinessProbe:
        httpGet:
          path: /healthz
          port: 80       # <-- this path doesn't exist
