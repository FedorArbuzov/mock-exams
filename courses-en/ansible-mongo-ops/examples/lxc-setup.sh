#!/usr/bin/env bash
# Reference only — run on the 16 GB host after ENVIRONMENT.md profile mongo exists.
set -euo pipefail

for i in 1 2 3; do
  name="mongo-0${i}"
  ip="192.168.59.$((9+i))"
  lxc launch ubuntu:22.04 "$name" --profile mongo
  lxc config device set "$name" eth0 ipv4.address="$ip"
  lxc config set "$name" limits.memory=2GiB
done
lxc restart mongo-01 mongo-02 mongo-03

test -f ~/.ssh/id_ed25519 || ssh-keygen -t ed25519 -N "" -f ~/.ssh/id_ed25519

for name in mongo-01 mongo-02 mongo-03; do
...
echo "SSH: ubuntu@192.168.59.10-12"
