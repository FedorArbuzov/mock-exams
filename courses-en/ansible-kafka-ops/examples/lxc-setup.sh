#!/usr/bin/env bash
# Reference only — run on the 16 GB host after ENVIRONMENT.md profile kafka exists.
set -euo pipefail

for i in 1 2 3; do
  name="kafka-0${i}"
  ip="192.168.57.$((9+i))"
  lxc launch ubuntu:22.04 "$name" --profile kafka
  lxc config device set "$name" eth0 ipv4.address="$ip"
  lxc config set "$name" limits.memory=2GiB
done
lxc restart kafka-01 kafka-02 kafka-03

test -f ~/.ssh/id_ed25519 || ssh-keygen -t ed25519 -N "" -f ~/.ssh/id_ed25519

for name in kafka-01 kafka-02 kafka-03; do
...
echo "SSH: ubuntu@192.168.57.10-12"
