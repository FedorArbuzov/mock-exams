import React from "react";

import {AbsoluteFill, useCurrentFrame} from "remotion";

import {Hammer, XCircle} from "lucide-react";

import {COLORS, FONTS} from "../../../../../shared/constants";

import {Caption} from "../../../../../shared/components/Caption";

import {Checkmark} from "../../../../../shared/components/Checkmark";

import {Panel} from "../../../../../shared/components/Panel";

import {progress01} from "../../../../../shared/utils/animations";



type Props = {text: string; durationInFrames: number};



const BAD_LOOP = ["for _, x := range xs {", "  s += x", "}"];



export const Scene4Builder: React.FC<Props> = ({text, durationInFrames}) => {

  const frame = useCurrentFrame();

  const leftIn = progress01(frame, 0, 22);

  const xIn = progress01(frame, 24, 42);

  const rightIn = progress01(frame, 46, 68);

  const labelIn = progress01(frame, 78, 98);



  return (

    <AbsoluteFill>

      <div

        style={{

          position: "absolute",

          left: "50%",

          top: 780,

          transform: "translate(-50%, -50%)",

        }}

      >

        <Panel width={960} height={720}>

          <div

            style={{

              position: "absolute",

              left: 240,

              top: 280,

              transform: `translate(-50%, -50%) translateY(${(1 - leftIn) * 12}px)`,

              opacity: leftIn,

              display: "flex",

              flexDirection: "column",

              alignItems: "center",

              gap: 16,

            }}

          >

            <div style={{position: "relative"}}>

              <div

                style={{

                  padding: "18px 22px",

                  borderRadius: 12,

                  border: `1.5px solid ${COLORS.red}66`,

                  background: "rgba(15, 23, 42, 0.55)",

                }}

              >

                {BAD_LOOP.map((line) => (

                  <div

                    key={line}

                    style={{

                      color: line.includes("+=") ? COLORS.red : COLORS.muted,

                      fontFamily: FONTS.mono,

                      fontSize: 38,

                      fontWeight: 700,

                      lineHeight: 1.5,

                      textDecoration: line.includes("+=") && xIn > 0.15 ? "line-through" : "none",

                      opacity: line.includes("+=") ? 0.55 + xIn * 0.45 : 1,

                    }}

                  >

                    {line}

                  </div>

                ))}

              </div>

              <div style={{position: "absolute", right: -18, top: -18, opacity: xIn}}>

                <XCircle size={42} color={COLORS.red} fill={COLORS.background} strokeWidth={1.8} />

              </div>

            </div>

          </div>



          <div

            style={{

              position: "absolute",

              left: "50%",

              top: 280,

              width: 1,

              height: 220,

              background: COLORS.border,

              opacity: (leftIn + rightIn) / 2,

            }}

          />



          <div

            style={{

              position: "absolute",

              left: 720,

              top: 280,

              transform: `translate(-50%, -50%) translateY(${(1 - rightIn) * 12}px)`,

              opacity: rightIn,

              display: "flex",

              flexDirection: "column",

              alignItems: "center",

              gap: 18,

            }}

          >

            <Hammer size={56} color={COLORS.green} strokeWidth={1.6} />

            <div

              style={{

                padding: "16px 24px",

                borderRadius: 12,

                border: `1.5px solid ${COLORS.green}88`,

                color: COLORS.green,

                fontFamily: FONTS.mono,

                fontSize: 42,

                fontWeight: 750,

              }}

            >

              strings.Builder

            </div>

            <Checkmark progress={rightIn} size={44} />

          </div>



          <div

            style={{

              position: "absolute",

              left: "50%",

              top: 540,

              transform: "translate(-50%, -50%)",

              opacity: labelIn,

              color: COLORS.cyan,

              fontFamily: FONTS.sans,

              fontSize: 38,

              fontWeight: 750,

            }}

          >

            use Builder

          </div>

        </Panel>

      </div>



      <Caption text={text} durationInFrames={durationInFrames} />

    </AbsoluteFill>

  );

};


