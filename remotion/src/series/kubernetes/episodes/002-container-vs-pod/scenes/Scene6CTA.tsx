...
export const Scene6CTA: React.FC<Props> = ({text, durationInFrames}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();

  const zoomOut = interpolate(frame, [0, durationInFrames], [1.08, 1], clamp);
  ...
  return (
    <AbsoluteFill>
      <AbsoluteFill style={{transform: `scale(${zoomOut})`, transformOrigin: "50% 45%"}}>
        <div
          style={{
            position: "absolute",
            inset: 0,
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 99,
                  background: color,
                  boxShadow: `0 0 ${6 + p.size}px ${color}`,
