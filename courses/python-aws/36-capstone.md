...
Построить **event-driven image pipeline** на стенде [`deploy/python-aws`](../../deploy/python-aws/README.md) — Python-аналог [aws-terraform image pipeline](../aws-terraform/23-final-project.md):

1. Upload JPEG в `s3://shop-uploads/uploads/`.
2. Lambda resize → thumb в `thumbs/`.
3. Metadata в DynamoDB (`shop-items` или отдельная table).
...
| # | Feature |
|---|---------|
| 1 | Handler `resize_image` в `shop_aws/lambda_handlers/` |
...
| 7 | pytest: handler unit test с moto + fake JPEG bytes |
| 8 | README ops section в `stack/CAPSTONE.md` (local) |
...
Адаптируйте из [aws-terraform handler](../aws-terraform/projects/image-pipeline/lambda/handler.py):
...
Поздравляем — трек **python-aws** (01–36) завершён. IaC версия: [aws-terraform/23-final-project](../aws-terraform/23-final-project.md).
