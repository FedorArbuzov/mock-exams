...
Полные развёрнутые главы: [17-interview-qa](17-interview-qa.md). Практика: [18-lab-mock-interview](18-lab-mock-interview.md).

**Стенд:** [`deploy/kafka`](../../deploy/kafka/README.md) · **Предшественник:** [`kafka-intermediate`](../kafka-intermediate/README.md) · **K8s:** [`kuber-intermediate/01-statefulset`](../kuber-intermediate/01-statefulset.md)
...
| 2 | Partition и key? | Ordered log per partition; key → stable partition | `hash(key)%N`; hot key; consumer parallelism capped by partitions |
| 3 | Offset? | Cursor в partition; commit в `__consumer_offsets` | Per-partition; not global; earliest/latest for new group |
| 4 | Leader и ISR? | Leader serves IO; ISR = caught-up replicas | `acks=all` + min ISR; election from ISR; URP when follower lags |
