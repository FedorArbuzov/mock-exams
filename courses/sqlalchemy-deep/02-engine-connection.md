...
`create_engine()` — **фабрика подключений** к PostgreSQL. Engine не держит постоянное соединение — **Connection pool** выдаёт connection на время операции.

## Что вы узнаете

- Engine vs Connection vs Session.
- `connect()`, `begin()`, transactions.
- `text()` для literal SQL.
...
Стенд sync: [`shop/db.py`](../../deploy/sqlalchemy/stack/shop/db.py).
