...
`.delay(*args, **kwargs)` — sugar over `.apply_async()`. Production code часто использует `apply_async` для queue, ETA, expires.
...
Меньше load на Redis. Подходит для audit logs.
...
Redis keys TTL — старые results исчезают.
...
`delay` = quick enqueue. `apply_async` = production options. `AsyncResult` polls backend. Don't block HTTP on `.get()`.

Далее: [07-rabbitmq-broker](07-rabbitmq-broker.md).
