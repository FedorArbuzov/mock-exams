...
Worker получил message и **умер** mid-task. Без правильных ack — message lost или duplicated.
...
Стенд включён — [`celery_app.py`](../../deploy/celery/stack/shop/celery_app.py).
...
Message discarded or sent to DLX (if configured). Стенд `amount=fail` — [18-lab-idempotent-order](18-lab-idempotent-order.md).
...
Production: `acks_late=True`, low prefetch, idempotent handlers. Reject permanent failures. Monitor DLQ.

Далее: [14-lab-acks](14-lab-acks.md).
