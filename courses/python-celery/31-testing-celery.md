...
Unit test не должен требовать RabbitMQ. **Eager mode** runs tasks synchronously in-process. Integration tests — real broker in CI.
...
- pytest fixtures for Celery app.
- Mocking `.delay()`.
...
`.delay()` executes **immediately** in same process — no broker.
...
| Ошибка | Fix |
|--------|-----|
| Eager in production | env flag tests only |
| Shared PROCESSED_IDS between tests | clear in fixture |
| Integration without timeout | `.get(timeout=5)` |

## Резюме

Unit: call task fn directly or eager mode. API: mock `.delay()`. Integration: broker in CI sparingly.

Далее: [32-lab-testing](32-lab-testing.md).
