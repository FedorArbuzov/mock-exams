...
> Для удобства есть **`mockctl`** — единый CLI на Go, заменяющий все скрипты в `scripts/`. См. [`mockctl/`](mockctl/README.md). Если используете его — все шаги ниже превращаются в `mockctl install` и `mockctl up`.
